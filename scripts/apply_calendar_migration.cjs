...
async function main() {
  console.log('Applying 20260904_google_calendar_integration.sql to hosted Supabase...');

  const statements = [
    // 1. Add columns to sessions
    `ALTER TABLE "sessions" ADD COLUMN IF NOT EXISTS "google_calendar_event_id" TEXT;`,
    `ALTER TABLE "sessions" ADD COLUMN IF NOT EXISTS "google_calendar_sync_status" TEXT NOT NULL DEFAULT 'not_connected';`,
    `ALTER TABLE "sessions" ADD COLUMN IF NOT EXISTS "google_calendar_synced_at" TIMESTAMPTZ;`,
    `ALTER TABLE "sessions" ADD COLUMN IF NOT EXISTS "google_calendar_last_error" TEXT;`,
    `CREATE INDEX IF NOT EXISTS "idx_sessions_calendar_event" ON "sessions"("google_calendar_event_id") WHERE "google_calendar_event_id" IS NOT NULL;`,

    // 2. Create candidate_google_tokens table
    ...
    `CREATE INDEX IF NOT EXISTS "idx_cgt_candidate_user_id" ON "candidate_google_tokens"("candidate_user_id");`,
    ...
    `CREATE POLICY "cgt_service_role_all" ON "candidate_google_tokens" FOR ALL TO service_role USING (true) WITH CHECK (true);`,

    // 3. RPC: upsert_candidate_calendar_token
    ...
    // 4. RPC: get_candidate_calendar_token
    ...
    // 5. RPC: disconnect_candidate_calendar
    ...
    // 6. RPC: update_session_calendar_sync
    ...
    BEGIN
        UPDATE "sessions"
        SET "google_calendar_event_id"   = COALESCE(p_event_id, "google_calendar_event_id"),
    ...
    // 7. RPC: get_candidate_interview_state
  ...
  console.log('✓ Migration executed successfully on hosted Supabase!');

  // Verify sessions table columns
  const cols = await prisma.$queryRawUnsafe(`
    SELECT column_name, data_type, column_default
    FROM information_schema.columns
    WHERE table_name = 'sessions' AND column_name LIKE 'google_calendar_%'
    ORDER BY column_name;
  `);
  console.log('Sessions calendar columns:', cols);

  // Verify candidate_google_tokens table
  const tokenTable = await prisma.$queryRawUnsafe(`
    SELECT table_name
    FROM information_schema.tables
    WHERE table_name = 'candidate_google_tokens';
  `);
  console.log('Candidate tokens table exists:', tokenTable.length > 0);
}
